      ...
      <div class="yw-el-table-wrap">
        <el-table :data="tableData"
          ...
          <el-table-column label="设备分类"
                           prop="device_class"></el-table-column>
          <el-table-column label="设备类型"
                           prop="device_type"></el-table-column>
          <el-table-column label="品牌"
                           prop="device_mfrs"></el-table-column>
          <el-table-column label="设备数量"
                           width="100" prop="number"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getPageData () {
      ...
      rbdeviceStatisticServiceFactory.selectDeviceBydeviceTypeDeviceMfrs(obj).then((res) => {
        this.tableData = res
      }).finally(() => {
        this.loading = false
    ...
    exportExcelData () {
      ...
      rbdeviceStatisticServiceFactory.downloadDeviceBydeviceTypeDeviceMfrs(obj).then((res) => {
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        let objectUrl = URL.createObjectURL(blob)
        // window.location.href = objectUrl
        let downLoadElement = document.createElement('a')
        downLoadElement.href = objectUrl
        downLoadElement.download = '各类型各品牌设备数量统计.xlsx'
