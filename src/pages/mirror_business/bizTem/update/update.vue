...
<!--           <el-form-item prop="fun_type" label="巡检类型：" class="add-tit" label-width="93px">
          ...
          <el-form-item prop="sys_type" label="系统类型：" class="add-tit" label-width="93px">
            <el-select v-model="newTemplate.sys_type" placeholder="请选择" class="list-sel add-con" :disabled="tableData.length > 0">
              <el-option
          ...
          <el-form-item prop="object_ids" label="关联对象：" class="add-tit" label-width="93px" v-else-if="newTemplate.mon_type === '1'">
            <el-input v-model="newTemplate.object_ids" placeholder="请选择设备" class="add-con moreCon" disabled :title="newTemplate.object_ids"></el-input>
...
<!--             <el-table-column
      ...
      <el-dialog title="新增业务指标" v-if="dialogFormVisible" :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="90%" class="addBiz" :before-close="handleClose">
        ...
        <div class="con">
          <el-form ref="business" label-width="100px" label-position="right" :rules="rules" :model="form" style="padding-left:20px;padding-right:20px;">
            ...
            <el-row>
              <el-col :span="24" >
                  <el-form-item label="统计分组" prop="biz_group" style="padding-left:5px;">
                    <el-select filterable style="width:326px;"
                      placeholder="请选择主题维度"
      ...
      packSend () {
        ...
        for (let i = 0; i < this.biz_calc_obj.length; i++) {
          if (this.biz_calc_state[i].show) {
            arr0.push(this.biz_calc_obj[i])
          }
        }
        obj.biz_calc_obj = JSON.stringify(arr0)
        let arr = []
    ...
    watch: {
      ...
      temState () {
        if (this.$store.state.homeStore.temState) {
          ...
          str = str.slice(0, -1)
          this.newTemplate.object_ids = str
