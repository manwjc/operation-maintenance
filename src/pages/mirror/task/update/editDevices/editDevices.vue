...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getTableData (obj) {
        let obj1 = Object.assign(this.$store.state.homeStore.template, {})
        let str = obj1.template_id
        obj.templateId = str
      ...
      initDevices () {
        let obj = Object.assign(this.$store.state.homeStore.template, {})
        if (obj.instanceId) {
          let arr = obj.devices_ip_more.split(',')
          let arr1 = obj.instanceId.split(',')
          for (let i = 0; i < arr.length; i++) {
            let obj1 = {
              name: arr[i],
              instanceId: arr1[i]
            }
            this.monitor.push(obj1)
          }
        }
      }
    },
