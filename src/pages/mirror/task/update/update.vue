  ...
  <div class="component-container">
    <header>
      <div class="head">
        ...
        <el-button class="sel-user" type="primary" @click="sel_user">选择用户</el-button>
      ...
      <div class="dataTime">
        ...
        <div v-if=' cycle == "" '>
          <el-input placeholder="请输入内容" class="input-creat-time" disabled prefix-icon="el-icon-time"
                    key="beg"></el-input>
        </div>
        ...
        <div v-if=' cycle == "自定义" ' style="position:relative">
          <el-input v-model="exec_time" placeholder="请输入cron表达式" class="input-creat-time zidingyi" @change="cronVal"
                    ref="cron" suffix-icon="el-icon-warning" key="zidingyi"></el-input>
          <span class="cron" ref="cronText">请输入6位数正确的cron表达式</span>
        </div>

        <div v-if=' cycle == "分钟" '>
          <el-input-number v-model="exec_time" controls-position="right" :min="1" :max="60" size="small"
                           key="minute"></el-input-number>
        </div>

        <div v-if=' cycle == "小时" '>
          <el-input-number v-model="exec_time" controls-position="right" :min="1" :max="60" size="small"
                           key="hour"></el-input-number>
        </div>
        ...
        <div v-if=' cycle == "周" '>
          <el-select v-model="zhou" placeholder="请选择" class="list-sel" style="width:100px" key="zhou">
            <el-option
        ...
        <div v-if=' cycle == "月" '>
          <el-select v-model="yue" placeholder="请选择" class="list-sel" style="width:100px" key="yue">
            <el-option
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button type="text" @click="editTemplate(scope.row)">模板编辑</el-button>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <!-- <el-button type="text" @click="editDevices(scope.row)">设备编辑</el-button> -->
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="goUser" class="tem-search1">确 定</el-button>
      </span>
    ...
    <footer>
      <el-button class="" type="primary" @click="save">修改</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      editTemplate (obj) {
        ...
        this.$router.push('update/editTemplate')
      },
      editDevices (obj) {
        if (obj.template_id !== -1) {
          this.$store.commit('editDevices', obj)
          this.$router.push('update/editDevices')
        } else {
      ...
      saveData () {
        let arr = []
        let id = this.$store.state.homeStore.update_task_id
        ...
        let arr1 = [obj, id]
        rbProjectDataServiceFactory.updateTask(arr1).then((res) => {
          if (res.task_id) {
            this.$message({
              message: '修改成功',
              type: 'success'
      ...
      initTime (str, str1) {
        if (str1 === 'MIN' || str1 === 'HOUR' || str1 === 'DEFINE') {
          ...
          this.yue = str.slice(0, index) + '号'
          this.exec_time = '2018-08-20 ' + str.slice(index + 1) + ':10'
        } else if (str1 === 'WEEK') {
          let index = str.indexOf('+')
          this.zhou = rbMirrorCommonService.getZhou(str.slice(0, index))
          this.exec_time = '2018-08-20 ' + str.slice(index + 1) + ':10'
        } else if (str1 === 'DAY') {
          this.exec_time = '2018-08-20 ' + str + ':10'
        } else if (str1 === '') {
          this.cycle = ''
          this.exec_time = ''
        }
      ...
      initArr (arr) {
        if (arr) {
          arr.forEach((item) => {
            if (item.device_instance_list[0].biz_code) {
              this.$set(item, 'devices_ip_more', item.device_instance_list[0].biz_name)
              this.$set(item, 'instanceId', item.device_instance_list[0].biz_code)
              this.$set(item, 'mon_type', '业务模板')
            } else {
              let str = ''
              let str1 = ''
              item.device_instance_list.forEach((item) => {
                str += item.device_name + ','
                str1 += item.device_id + ','
              })
              str = str.slice(0, -1)
              str1 = str1.slice(0, -1)
              this.$set(item, 'devices_ip_more', str)
              this.$set(item, 'instanceId', str1)
              this.$set(item, 'mon_type', '系统模板')
              // this.$set(item, 'device_instance_list', item.device_instance_list)
            }
          })
          return arr
        } else {
      ...
      initData () {
        if (this.$store.state.homeStore.update_task_id) {
          let id = this.$store.state.homeStore.update_task_id
          rbProjectDataServiceFactory.getTaskDetail(id).then((res) => {
            this.name = res.name
            this.template.name = res.name
            this.template.receivers = this.initReceivers(res.receivers)
            this.template.type = rbMirrorCommonService.getTaskType1(res.type)
            this.cycle = rbMirrorCommonService.getCycleType(res.cycle)
            this.initTime(res.exec_time, res.cycle)
            this.tableData = this.initArr(res.object_list)
          })
        }
      },
      // cron表达式
    ...
    watch: {
      ...
      cycle (newValue, oldValue) {
        if (oldValue === '分钟' || oldValue === '小时' || oldValue === '月' || oldValue === '周' || oldValue === '日') {
          if (newValue === '自定义') {
            this.$nextTick(() => {
              this.exec_time = ''
            })
