        ...
        <el-row type="flex" justify="center">
          ...
          <el-col :span="7">
            <div class="devices-one">机房位置：
              <el-select v-model="machine" placeholder="请选择" class="list-sel" clearable>
                <el-option
                  v-for="item in room"
                  :key="item.id"
                  :label="item.name"
...
<!--         <div class="devices-one">设备名称：
    ...
    <section style="height:86.72%">
      <div class="selected-head">
        ...
        <el-button type="primary" icon="el-icon-d-arrow-right" class="tem-search mirror_but" @click="addItems" style="margin-left:0">添加</el-button>
        <el-button type="primary" icon="el-icon-delete" class="tem-search mirror_but" @click="clearAll">清空</el-button>
      </div>
...
<script>
  import {equipType, room} from '../../../task/config/options.js'
  ...
  export default {
    ...
    computed: {
      temDevices () {
        return this.$store.state.homeStore.temDevi
      }
    },
    methods: {
      // 多选项选中
      ...
      selected () {
        if (this.monitor.length <= 0) {
          ...
          // this.$store.commit('editDevices1', this.packTaskDevices())
          // this.$store.commit('adDevices',this.monitor)
          let arr = this.monitor.concat([])
          this.$store.commit('temDevices', arr)
      ...
      initDevices () {
        if (this.$store.state.homeStore.temDevi) {
