<template>
  <div class="component-container">
    ...
    <div class="body-container">
      <el-steps :active="active" finish-status="success" align-center v-if="newTemplate.sys_type !== 'THEME_CALC'">
      ...
      <el-steps :active="active" finish-status="success" align-center v-else>
        <el-step title="填写模板信息">
        </el-step>
        <el-step title="设置指标">
        </el-step>
...
<!--           <el-form-item prop="fun_type" label="巡检类型：" class="add-tit" label-width="93px">
            <el-select v-model="newTemplate.fun_type" placeholder="请选择" class="list-sel add-con" disabled>
              <el-option
                v-for="item in fun_type_options"
                :key="item.value"
                :label="item.label"
                :value="item.value">
              </el-option>
          ...
          <el-form-item prop="mon_type" label="监控类型：" class="add-tit" label-width="93px">
            <el-select v-model="newTemplate.mon_type" placeholder="请选择" class="list-sel add-con" @change="monChange()" disabled>
              <el-option
                v-for="item in mon_type_options"
                :key="item.value"
                :label="item.label"
                :value="item.value">
              </el-option>
          ...
          <el-form-item prop="sys_type" label="系统类型：" class="add-tit" label-width="93px">
            <el-select v-model="newTemplate.sys_type" placeholder="请选择" class="list-sel add-con" disabled>
              <el-option
          ...
          <el-form-item prop="object_ids" label="关联对象：" class="add-tit" label-width="93px" v-else-if="newTemplate.mon_type === '1'">
            <el-input v-model="newTemplate.object_ips" placeholder="请选择设备" class="add-con moreCon" disabled :title="newTemplate.object_ips"></el-input>
            <el-button class="mirror_but" type="primary" @click="goDevices()" style="position: absolute;left: 215px;top: 5px;">设备管理</el-button>
          </el-form-item>
        </el-form>
        <footer>
      ...
      <div class="add-table" v-show="active == 1 ? true:false">
        <div v-if="newTemplate.sys_type !== 'THEME_CALC'">
        ...
        <div v-else>
          <theme-calce @stepChange="stepChange(arguments, 'msg')" :themeOptions="theme_data_options" :template1="template" :newTemplate1="newTemplate" :tableData1="tableData"></theme-calce>
        </div>
      </div>
...
<!--             <el-table-column
              ...
              >
              <template slot-scope="scope">
                <el-select v-model="triggerData[scope.$index].express" placeholder="请选择" class="add-exp" v-if="triggerData[scope.$index].item_id !== ''">
                  <el-option
                    v-for="item in trigger_options.expressionList"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value">
                  </el-option>
      ...
      <el-dialog title="新增系统指标" v-if="dialogFormVisible" :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="90%" class="addBiz" :before-close="handleClose">
...
<script>
  ...
  import themeCalce from '../components/themeCalce.vue'
  ...
  export default {
    ...
    methods: {
      ...
      dataAdd (str, str1, num) {
        // str: yeild date str1: zbxA num: 第几项
        if (str === 'field') {
      ...
      initDim () {
        this.form = {calc_type: '2', biz_is_zero: '1'}
        this.name2 = ''
        this.key2 = ''
        this.weidu_time_list = []
        this.weidu_other_list = []
      ...
      formateTrigger (arr) {
        ...
        arr1.forEach((item) => {
          // item.item_id = item.express + item.match
          item.status = 'ON'
        })
        return arr1
      },
      ...
      updateInit () {
        rbProjectDataServiceFactory.detail(this.$store.state.homeStore.update_template_id).then((res) => {
          let arr = JSON.parse(JSON.stringify(res.item_list))
          arr.forEach((item) => {
            rbProjectDataServiceFactory.getThemeDetail(item.biz_theme_id).then((obj) => {
              item.theme_name = obj.theme_name
          ...
          this.name1 = res.name
          this.newTemplate.name = res.name
          this.newTemplate.description = res.description
          this.newTemplate.mon_type = res.mon_type
          this.newTemplate.fun_type = res.fun_type
          this.newTemplate.sys_type = res.sys_type
          this.device_obj = this.packDeviceObj(res.object_list)
          if (this.newTemplate.mon_type === '1') {
            let deviceIds = ''
            let deviceIps = ''
            if (res.object_list) {
              res.object_list.forEach((item) => {
                deviceIds = deviceIds + (item ? item.device_id : '') + ','
                deviceIps = deviceIps + (item ? item.device_name : '') + ','
              })
            }
            this.newTemplate.object_ids = deviceIds.slice(0, -1)
            this.newTemplate.object_ips = deviceIps.slice(0, -1)
          } else {
            this.newTemplate.object_ids = res.object_list[0] ? res.object_list[0].biz_code : ''
          }
          this.template.name = res.name
          this.template.template_id = res.template_id
        })
        rbProjectDataServiceFactory.getTemplateTrigger(this.$store.state.homeStore.update_template_id).then((res) => {
