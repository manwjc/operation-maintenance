      ...
      <div class="yw-el-table-wrap">
        <el-table class="yw-el-table"
          ...
          <el-table-column label="规则类型"
                           width="160">
            <template slot-scope="scope">
              {{scope.row.deviceRelationType === '2' ? '不同设备' : '相同设备'}}
            </template>
          </el-table-column>
          ...
          <el-table-column label="主要告警总数"
                           width="140">
            <template slot-scope="scope">
              {{scope.row.primaryAlertCount ? scope.row.primaryAlertCount: '0'}}
            </template>
          </el-table-column>
          <el-table-column label="次要告警总数"
                           width="140">
            <template slot-scope="scope">
<!--              <a @click="getLogs(scope.row.id)">{{scope.row.secondaryAlertCount ? scope.row.secondaryAlertCount: '0'}}</a>-->
              {{scope.row.secondaryAlertCount ? scope.row.secondaryAlertCount: '0'}}
            </template>
          </el-table-column>
          <el-table-column label="最新告警时间"
                           width="180">
            <template slot-scope="scope">
              {{scope.row.newestSecondaryDate | formatDate}}
            </template>
          </el-table-column>
    ...
    <!-- dialog -->
    <DialogPrimarySecondary :dialogMsg="dialogMsg"
                  @closeDialog="closeDialog"
                  v-if="dialogMsg.dialogVisible"></DialogPrimarySecondary>
    <!-- dialog -->
  </div>
</template>
...
<script>
  ...
  export default {
    name: '/mirror_alert/primarySecondary',
    mixins: [QueryObject, optionx],
    components: {
      DialogPrimarySecondary: () => import('../dialog-primary-secondary.vue'),
    ...
    methods: {
      ...
      query(activePagination = false) {
        ...
        rbAlertPrimarySecondaryServiceFactory.getprimarySecondaryList(this.queryParams).then((data) => {
      ...
      delBatch() {
        ...
        rbAlertPrimarySecondaryServiceFactory.delete(ids).then((data) => {
          this.query()
        })
      },
      del(id) {
        rbAlertPrimarySecondaryServiceFactory.delete([id]).then((data) => {
          this.query()
        })
      },
      ...
      openBatch() {
        ...
        if (!openFlag) {
          this.$alert('已启用主次规则无法再次启用', '警告', {
            confirmButtonText: '确定'
          })
          return
        }
        this.$confirm('确认启用?').then(() => {
          rbAlertPrimarySecondaryServiceFactory.start(ids).then((data) => {
            this.query()
          })
        })
      },
      open(id) {
        this.$confirm('确认启用?').then(() => {
          rbAlertPrimarySecondaryServiceFactory.start([id]).then((data) => {
            this.query()
          })
        })
      },
      ...
      closeBatch() {
        ...
        if (!closeFlag) {
          this.$alert('已停用主次规则无法再次停用', '警告', {
            confirmButtonText: '确定'
          })
          return
        }
        this.$confirm('确认停用?').then(() => {
          rbAlertPrimarySecondaryServiceFactory.stop(ids).then((data) => {
            this.query()
          })
        })
      },
      close(id) {
        this.$confirm('确认停用?').then(() => {
          rbAlertPrimarySecondaryServiceFactory.stop([id]).then((data) => {
            this.query()
          })
        })
      },
