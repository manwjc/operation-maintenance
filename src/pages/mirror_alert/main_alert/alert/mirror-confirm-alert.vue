  ...
  methods: {
    ...
    initAlertSum () {
      ...
      if (this.$route.query.alertType === 'main') {
        ...
        this.search.confirm_date_from = this.requestData.confirmTimeRangeStart
        this.search.confirm_date_to = this.requestData.confirmTimeRangeEnd
        this.this.search.confirmer = this.requestData.confirmStaff
      }
      this.getAlertSum()
    },
    getAlertSum () {
      rbAlertHomeServicesFactory.getHomeAlertCount(this.packInit(), 'confirm').then((res) => {
    ...
    // 获取列表数据相关方法
    getTableData (obj) {
      rbAlertHomeServicesFactory.getHomeAlertList(obj, 'confirm').then((res) => {
    ...
    packInit () {
      let obj = {
        ...
        'confirmStaff': this.search.confirmer ? this.search.confirmer : ''
      }
      return obj
    },
    // 封装时间戳
    ...
    resourceExport () {
      ...
      rbAlertHomeServicesFactory.ExportHomeAlertData(this.packInit(), 'confirm').then((res) => {
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        let objectUrl = URL.createObjectURL(blob)
        // window.location.href = objectUrl
        let downLoadElement = document.createElement('a')
        downLoadElement.href = objectUrl
        downLoadElement.download = '(工作台)确认告警数据列表.xlsx'
    ...
    voiceNotify () {
      rbAlertVoiceNotifyServicesFactory.getAlertVoiceNotify().then((result) => {
        if (result && result.isOpen === 1) {
          rbAlertHomeServicesFactory.getHomeAlertVoiceContent('confirm').then((res) => {
            if (res && res !== 'error') {
