...
export default {
  ...
  methods: {
    ...
    query (activePagination = false) {
      ...
      //自己查询接口
      this.setParams(activePagination);
      rbAlertNetworkStrategyFactory.getPublicNetData(this.queryParams).then((res) => {
        this.loading = false;
        this.result = res.result
        this.total = res.count
        this.$emit('updateTabData', this.total, 'publicNet');
      }).finally(() => {
        this.loading = false;
      })
    },
    ...
    //导出
    exportOut () {
      rbAlertNetworkStrategyFactory.exportPublicNetData({ 'keyword': this.keyword }).then((res) => {
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        let objectUrl = URL.createObjectURL(blob)
        let downLoadElement = document.createElement('a')
        downLoadElement.href = objectUrl
        downLoadElement.download = '网络策略-公网.xlsx'
