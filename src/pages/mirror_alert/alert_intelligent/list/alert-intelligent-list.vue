      ...
      <div class="yw-el-table-wrap">
        <el-table border
          ...
          <el-table-column v-for="itemData in checkColumns"
                           ...
                           sortable
                           :show-overflow-tooltip="true"
                           :width="flexColumnWidth(itemData.keyCode)"></el-table-column>
          <el-table-column label="收敛告警数"
                           align="center"
                           width="100px">
            <template slot-scope="scope">
              <el-button style="font-size: 16px;"
                          type="text"
                          >{{scope.row.intelligentCount}}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <el-dialog class="yw-dialog"
      ...
      <section class="yw-dialog-main">
        <el-form class="yw-form is-required"
                 label-width="90px" :model="clean"
                 ref="formClean" :rules="cleanRules">
...
export default {
  ...
  methods: {
    ...
    getAlertSum () {
      rbAlertIntelligentServicesFactory.alertIntelligentOverview(this.packInit()).then((res) => {
    ...
    // 获取列表数据相关方法
    getTableData (obj) {
      rbAlertIntelligentServicesFactory.queryAlertIntelligent(obj, '').then((res) => {
        this.activityAlertData = this.packData(res.result)
        this.total = res.count
      }).finally(() => {
        this.loading = false
    ...
    resourceExport () {
      ...
      rbAlertIntelligentServicesFactory.exportAlertIntelligentData(this.packInit()).then((res) => {
        let blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        let objectUrl = URL.createObjectURL(blob)
        // window.location.href = objectUrl
        let downLoadElement = document.createElement('a')
        downLoadElement.href = objectUrl
        downLoadElement.download = '告警智能收敛数据表.xlsx'
    ...
    toAlerts (alertId) {
      this.$router.push({
        ...
        query: {
          alertType: 'intelligent',
          intelligentAlertId: alertId
        }
      })
    }
  }
