  ...
  <div>
    <el-form class="components-condition yw-form"
      ...
      <el-form-item label="过滤器">
        <el-select v-model="filterId"
                   @change="filterChange"
                   clearable
                   filterable>
          <el-option :key="valIndex"
                     :label="val.name"
                     :value="val.id"
                     v-for="(val, valIndex) in filterData"></el-option>
        </el-select>
        <div class="yw-filter-wrap">
          <el-button class="filter-add-btn"
                     type="text"
                     @click="goFilter()"
                     icon="el-icon-circle-plus-outline"></el-button>
          <div class="filter-list"
               v-if="filterSceneData.length > 0">
            <a class="filter-link"
               href="#"
               @click="getFilterData(scene.condition, scene.sceneId)"
               v-for="(scene, sIndex) in filterSceneData">{{scene.sceneName}}({{scene.count}})</a>
          </div>
        </div>
    ...
    <el-form class="yw-form">
      <div class="table-operate-wrap clearfix">
        ...
        <el-button class="btn-icons-wrap"
                   ...
                   v-if="alertType === 'activity'">转派</el-button>
        <el-button class="btn-icons-wrap"
                   type="text"
                   icon="el-icon-circle-check"
                   @click="confirmDialog"
                   v-if="alertType === 'activity'">确认</el-button>
        <el-button class="btn-icons-wrap"
                   type="text"
        ...
        <el-button class="btn-icons-wrap"
                   ...
                   v-if="alertType === 'activity'">语音通知</el-button>
      </div>
      <div class="yw-el-table-wrap">
        <el-table border
          ...
          <el-table-column prop="order_id"
                           ...
                           sortable>
            <template slot-scope="scope">
              <el-button @click="gotoBPM(scope.row.order_id)"
                         type="text"
    ...
    <el-dialog class="yw-dialog"
      ...
      <section class="yw-dialog-main">
        <el-form class="yw-form"
                 label-width="84px" v-if="confirm.confirmDialog">
...
import rbAlertIntelligentServicesFactory from 'src/services/alert/rb-alert-intelligent-services.factory.js'
...
export default {
  ...
  methods: {
    ...
    getAlertSum () {
      let code = this.alertType === 'activity' ? '0' : '1'
    ...
    packInit () {
      ...
      let obj = {
        ...
        'curMoniTimeFrom': this.search.cur_moni_time_from ? this.search.cur_moni_time_from : '',
        'curMoniTimeTo': this.search.cur_moni_time_to ? this.search.cur_moni_time_to : '',
    ...
    resourceExport () {
      ...
      if (this.filterFlag) {
        rbAlertFilterServicesFactory.ExportFilterGridData(page).then((res) => {
    ...
    voiceNotify () {
      rbAlertVoiceNotifyServicesFactory.getAlertVoiceNotify().then((result) => {
        if (result && result.isOpen === 1) {
          rbAlertVoiceNotifyServicesFactory.getAlertVoiceNotifyContent(result).then((res) => {
            if (res && res !== 'error') {
